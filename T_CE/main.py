...
parser.add_argument('--flip_rate', 
	type = float, 
	help = 'flip rate', 
	default = 0)
parser.add_argument('--flip_num_gradual', 
	type = int, 
	default = 100000, 
...
def eval(model, valid_loader, best_loss, count):
	...
	for user, item, label, noisy_or_not in valid_loader:
		...
		loss, _, _ = loss_coteaching(prediction, label, select_rate_schedule(count), flip_rate_schedule(count), noisy_or_not)
	...
	if epoch_loss < best_loss:
		...
		if args.out:
			...
			torch.save(model, '{}{}_{}-{}-{}-{}.pth'.format(config.model_path, config.model, args.flip_rate, args.flip_num_gradual, args.forget_rate, args.num_gradual))
...
def test(model, test_data_pos, user_pos):
	top_k = args.top_k
	model.eval()
	precision, recall, NDCG, MRR = evaluate.test_all_users(model, 4096, item_num, test_data_pos, user_pos, top_k)
...
count, best_hr = 0, 0
best_loss = 1e9
pure_ratio_select_list = []
pure_ratio_flip_list = []
...
for epoch in range(args.epochs):
	...
	for user, item, label, noisy_or_not in train_loader:
		...
		model.zero_grad()
		prediction = model(user, item)
		loss, pure_ratio_select, pure_ratio_flip = loss_coteaching(prediction, label, select_rate_schedule(count), flip_rate_schedule(count), noisy_or_not)
		loss.backward()
		optimizer.step()

		if count % args.eval_freq == 0 and count != 0:
			pure_ratio_select_list.append('%.2f' % pure_ratio_select)
			pure_ratio_flip_list.append('%.2f' % pure_ratio_flip)
			print("epoch: {}, iter: {}, loss:{}".format(epoch, count, loss))
			# if count % 10000==0:
			print("pure_ratio_select", pure_ratio_select_list)
			print("pure_ratio_flip", pure_ratio_flip_list)
...
# test(model, test_data_pos, user_pos)
print("############################## Training End. ##############################")
test_model = torch.load('{}{}_{}-{}-{}-{}.pth'.format(config.model_path, config.model, args.flip_rate, args.flip_num_gradual, args.forget_rate, args.num_gradual))
test_model.cuda()
